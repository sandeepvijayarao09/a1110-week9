...
class BugHoundAgent:
    ...
    def analyze(self, code_snippet: str) -> List[Dict[str, str]]:
        ...
        # Validate quality: if most issues are missing a message, the LLM
        # didn't follow the schema and the output isn't trustworthy.
        if issues and sum(1 for i in issues if not i.get("msg", "").strip()) > len(issues) // 2:
            self._log("ANALYZE", "LLM returned too many issues with empty messages. Falling back to heuristics.")
            return self._heuristic_analyze(code_snippet)
