...
check("mixed: all three heuristics fire (Reliability, Code Quality, Maintainability)",
      {"Reliability", "Code Quality", "Maintainability"} == issue_types, f"got {issue_types}")
result = agent.run(CLEAN_CODE)
check("clean code produces no issues", len(result["issues"]) == 0, f"got {len(result['issues'])} issue(s)")
result = agent.run(EMPTY_INPUT)
check("empty input handled gracefully (no crash)", isinstance(result, dict) and "issues" in result)

print("\n=== 2. Fix Proposal (heuristic mode) ===")
agent = BugHoundAgent(client=None)
result = agent.run(BARE_EXCEPT)
check("bare except fix replaces except: with except Exception", "except Exception" in result["fixed_code"])
result = agent.run(PRINT_SPAM)
check("print fix introduces logging.info", "logging.info(" in result["fixed_code"])
check("print fix adds import logging", "import logging" in result["fixed_code"])
result = agent.run(CLEAN_CODE)
check("clean code is returned unchanged", result["fixed_code"].strip() == CLEAN_CODE.strip())

print("\n=== 3. Risk Assessment ===")
risk = assess_risk(original_code="print('hi')\n", fixed_code="", issues=[])
check("empty fix is always high risk / no autofix", risk["level"] == "high" and risk["should_autofix"] is False)
risk = assess_risk(
...
check("high-severity issue keeps score <= 60", risk["score"] <= 60, f"score={risk['score']}")
check("high-severity issue blocks autofix", risk["should_autofix"] is False)
original = "def f():\n    print('hi')\n"
fixed_with_import = "import logging\n\ndef f():\n    logging.info('hi')\n"
risk = assess_risk(original_code=original, fixed_code=fixed_with_import,
                   issues=[{"type": "Code Quality", "severity": "Low", "msg": "print"}])
check("new import surfaces in risk reasons", any("import" in r.lower() for r in risk["reasons"]))
check("score < 90 when new import added", risk["score"] < 90, f"score={risk['score']}")
risk = assess_risk(
...
check("heuristic fallback still detects issues after non-JSON LLM", len(result["issues"]) > 0)
...
check("heuristic fixer still produces non-empty fix", len(result["fixed_code"].strip()) > 0)
